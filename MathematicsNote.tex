...
\subsection{空间中直线与直线的关系}
    ...
    对于两条不重合的直线$l_1$~$l_2$，若其所成角满足$\theta=0$\,\,\,，那么这两条直线平行，记作：$l_1\parallel l_2$\\[3mm]
    对于两条不重合的直线$l_1$~$l_2$，若其所成角满足$\theta=\dfrac{\pi}{2}$，那么这两条直线垂直，记作：$l_1\perp l_2$
...
\subsection{空间中直线与平面的关系}
    ...
    对于平面$\alpha$外的直线$l$，若其所成角满足$\theta=0\,\,\,$，那么直线和平面平行，记作：$l\,\parallel\alpha$\\[3mm]
    对于平面$\alpha$外的直线$l$，若其所成角满足$\theta=\dfrac{\pi}{2}$，那么直线和平面垂直，记作：$l\perp\alpha$\\[7mm]
    \textbf{线面平行判定定理：}对于任意平面$\alpha$，如果平面$\alpha$外存在直线$l_1$，同时平面$\alpha$上存在直线$l_2$，\\
    满足直线$l_1$平行于直线$l_2$，那么直线$l_1$与平面$\alpha$平行。\\[4mm]
    线面平行判定定理的证明：\\[3mm]
    使用反证法，假设直线$l_1$和平面$\alpha$不平行，两者相交于点$O$\\[3mm]
    过点$O$作直线$l_2$的平行线$l_0$，显然$l_2\parallel l_0$，显然$l_1\parallel l_0$\\[3mm]
    过点$O$的直线$l_0$和过点$O$的直线$l_1$平行，但两条平行直线却存在交点\\[3mm]
    矛盾，故假设不成立，因此直线$l_1$和平面$\alpha$平行\\[7mm]
    \textbf{线面平行性质定理：}对于任意直线$l$，若存在平面$\alpha$平行于直线$l$，且存在平面$\beta$经过于直线$l$，
    那么平面$\alpha$和平面$\beta$的交线$l_0$平行于直线$l$。\\[4mm]
    线面平行性质定理的证明：\\[3mm]
    使用反证法，假设直线$l$与交线$l_0$不平行，两者相交于点$O$\\[3mm]
    由于交线$l_0$过点$O$，所以平面$\alpha$过点$O$，所以平面$\beta$过点$O$\\[3mm]
    由于平面$\alpha$过点$O$，同时直线$l$过点$O$，但是平面$\alpha$平行于直线$l$\\[3mm]
...
\subsection{空间中平面与平面的关系}
    ...
    对于两个不重合的平面$\alpha$和平面$\beta$，若其所成角满足$\theta=0$\,\,，那么两个平面平行，记作$\alpha\parallel\beta$\\[3mm]
    对于两个不重合的平面$\alpha$和平面$\beta$，若其所成角满足$\theta=\dfrac{\pi}{2}$，那么两个平面平行，记作$\alpha\perp\beta$\\[6mm]
...
\subsection{圆柱}
    \textbf{圆柱：}将一个矩形绕一条边旋转一周所形成的几何体。\\[3mm]
...
\subsection{圆锥}
    \textbf{圆锥：}将一个直角三角形绕一条直角边旋转一周所形成的几何体。\\[3mm]
...
\subsection{几何概型}
    ...
    在几何概型中，我们可以将一个事件理解为一块区域，而事件的概率只与区域的几何度量有关，与区域的位置和形状无关。\vspace{3pt}
