...
\subsubsection{排列组合问题\rnum{1}-$07$}
    排列组合问题\rnum{1}-$07$背景：存在$\lambda$个约束集合$X_1\sum X_\lambda$，其中的元素必须保持相对顺序固定。\\[3mm]
    排列组合问题\rnum{1}-$07$公式：
    \begin{large}
        \begin{equation*}
...
\subsubsection{斯特林数}
    ...
    考虑空集数$>=2$的情况：
    \begin{align}
        \mathrm{C}_m^2\cdot(m-2)^n=\mathrm{C}_2^2\cdot T(2)+\mathrm{C}_3^2\cdot T(3)+\mathrm{C}_4^2\cdot T(4)+\cdots+\mathrm{C}_m^1\cdot T(m)=\sum_{h=1}^m\mathrm{C}_h^2\cdot T(h)
    \end{align}\\
...
\newpage
    ...
    \begin{align}
        ...
        &T(2)\cdot\Big[(-1)^0\cdot \mathrm{C}_3^0+(-1)^1\cdot \mathrm{C}_3^1+(-1)^3\cdot \mathrm{C}_3^2+(-1)^3\cdot \mathrm{C}_3^3\Big]+\\[3mm]
        &\vdots\\[3mm]
        &T(m)\cdot\Big[(-1)^0\cdot \mathrm{C}_m^0+(-1)^1\cdot \mathrm{C}_m^1+(-1)^3\cdot \mathrm{C}_m^2+\cdots+(-1)^m\cdot \mathrm{C}_m^m\Big]
    \end{align}\\
    接下来将用累加符号表示变形后的右侧式子。
...
\subsubsection{排列组合问题\rnum{2}-$01$}
    排列组合问题\rnum{2}-$01$背景：将$n$个\textbf{不同的}球放入$m$个\textbf{相同的}盒中，保证至少$1$个。\\[3mm]
    排列组合问题\rnum{2}-$01$公式：
    \begin{large}
        \begin{equation*}
...
\subsubsection{排列组合问题\rnum{2}-$02$}
    排列组合问题\rnum{2}-$02$背景：将$n$个\textbf{不同的}球放入$m$个\textbf{不同的}盒中，保证至少$1$个。\\[3mm]
    排列组合问题\rnum{2}-$02$公式：
    \begin{large}
        \begin{equation*}
...
\subsubsection{排列组合问题\rnum{2}-$03$}
    排列组合问题\rnum{2}-$03$背景：将$n$个\textbf{相同的}球放入$m$个\textbf{不同的}盒中，保证盒$m_i$个中至少$x_i$个。\\[3mm]
    排列组合问题\rnum{2}-$03$背景：
    \begin{large}
        \begin{equation*}
            P=\mathrm{C}_{n-\sum_{i=1}^m[x_i-1]-1}^m
        \end{equation*}
    \end{large}\\
    考虑以下示意图：\vspace{3pt}
    \begin{figure}[h]
        ...
        \end{center}
    \end{figure}\\
    首先对于第$i$个盒子$m_i$放入$x_i-1$个小球：\\[3mm]
    第$1$个盒子要求至少$2$个，则放入$1$个小球。\\[3mm]
    第$2$个盒子要求至少$3$个，则放入$2$个小球。\\[3mm]
    第$3$个盒子要求至少$4$个，则放入$3$个小球。\\[5mm]
    然后在剩余小球除去两侧的共$n-\sum_{i=1}^{m}[x_i-1]-1$个缝隙中任意插入$m_i-1$块板。\\[3mm]
