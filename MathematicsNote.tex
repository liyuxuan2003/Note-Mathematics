...
\subsubsection{组合数公式$02$}
    ...
    \begin{large}
        \begin{equation*}
            \mathrm{C}_n^m=\frac{\mathrm{P}_n^k}{\mathrm{P}_m^k}\cdot\mathrm{C}_{n-k}^{m-k}
        \end{equation*}
    \end{large}\\
    组合数公式$02$（弱化形式）：
    \begin{large}
    ...
    \begin{align}
        ...
        &=\frac{\mathrm{P}_n^k}{\mathrm{P}_m^k}\cdot\mathrm{C}_{n-k}^{m-k}
    \end{align}\\
    特别的，令$k=1$可以得到：
    \begin{align}
        \mathrm{C}_n^m
        &=\frac{\mathrm{P}_n^k}{\mathrm{P}_m^k}\cdot\mathrm{C}_{n-k}^{m-k}\\[3mm]
        &=\frac{\mathrm{P}_n^1}{\mathrm{P}_m^1}\cdot\mathrm{C}_{n-1}^{m-1}\\[3mm]
...
\subsubsection{组合数公式$03$}
    ...
    \begin{large}
        \begin{equation*}
            \mathrm{C}_n^m=\frac{\mathrm{P}_n^k}{\mathrm{P}_{n-m}^k}\cdot\mathrm{C}_{n-k}^m
        \end{equation*}
    \end{large}\\
    组合数公式$03$（弱化形式）：
    \begin{large}
    ...
    \begin{align}
        ...
        &=\frac{\mathrm{P}_n^k}{\mathrm{P}_{n-m}^k}\cdot\mathrm{C}_{n-k}^{m-k}
    \end{align}\\
    特别的，令$k=1$可以得到：
    \begin{align}
        \mathrm{C}_n^m
        &=\frac{\mathrm{P}_n^k}{\mathrm{P}_{n-m}^k}\cdot\mathrm{C}_{n-k}^{m-k}\\[3mm]
        &=\frac{\mathrm{P}_n^1}{\mathrm{P}_{n-m}^1}\cdot\mathrm{C}_{n-1}^{m-1}\\[3mm]
...
\subsubsection{组合数公式$04$}
    ...
    \begin{large}
        \begin{equation*}
            \mathrm{C}_n^m=\frac{\mathrm{P}_{m+k}^k}{\mathrm{P}_{n-m}^k\cdot\mathrm{C}_n^{m+k}}
        \end{equation*}
    \end{large}\\
    组合数公式$04$（弱化形式）：
    \begin{large}
    ...
    \begin{align}
        ...
        &=\frac{\mathrm{P}_{m+k}^k}{\mathrm{P}_{n-m}^k}\cdot\mathrm{C}_n^{m+k}
    \end{align}\\
    特别的，令$k=1$可以得到：
    \begin{align}
        \mathrm{C}_n^m
        &=\frac{\mathrm{P}_{m+k}^k}{\mathrm{P}_{n-m}^k}\cdot\mathrm{C}_n^{m+k}\\[3mm]
        &=\frac{\mathrm{P}_{m+k}^1}{\mathrm{P}_{n-m}^1}\cdot\mathrm{C}_n^{m+1}\\[3mm]
...
\subsubsection{组合数公式$05$}
    ...
    \begin{align}
        ...
        &\mathrm{C}_n^{m-1}=\frac{m}{n-m+1}\cdot\mathrm{C}_n^{m}\\[3mm]
    \end{align}\\
    进行一些变形可以得到：
    \begin{align}
        \mathrm{C}_n^m
...
\subsubsection{组合数公式$06$}
    ...
    \end{align}\\
    左式等于右式，故组合数公式$06$成立。\vspace{15pt}

\subsection{复杂的排列问题}
    我们首先约定符号和背景，以便后续讨论。\\[3mm]
    设共有$p$个元素，需对其进行排列。\\[3mm]
    其中的$p$个元素可以分至若干集合中，包含一个普通集合和若干个约束集合。\\[3mm]
...
\subsubsection{复杂排列问题$01$}
    复杂排列问题$01$背景：存在一个约束集合$X$，其中的元素必须相邻。\\[3mm]
    复杂排列问题$01$公式：
    \begin{large}
        \begin{equation*}
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            \end{tikzpicture}
            \caption{复杂排列问题$01$的示意图}
        \end{center}
    \end{figure}\\
    整体上考虑，共有$n+1$个元素或组块进行全排列，其排列数为$\mathrm{P}_{n+1}^{n+1}$。\\[3mm]
    约束集合中共有$x$个元素，其在组块内共有$x$个位置，其排列数为$\mathrm{P}_{x}^{x}$。\\[3mm]

\subsubsection{复杂排列问题$02$}
    复杂排列问题$02$背景：存在一个约束集合$X$，其中的元素必须隔开。\\[3mm]
    复杂排列问题$02$公式：
    \begin{large}
        \begin{equation*}
           P=\mathrm{P}_{x}^{n+1}\cdot\mathrm{P}_n^n
        \end{equation*}
    \end{large}\\ 
    将约束集合$X$中的元素进行插空：
    \begin{figure}[h]
        \begin{center}
            ...
            \end{tikzpicture}
            \caption{复杂排列问题$02$的示意图}
        \end{center}
    \end{figure}\\
    整体上考虑，共有$n$个普通元素进行全排列，其排列数为$\mathrm{P}_n^n$。\\[3mm]
    约束集合中共有$x$个元素，其在插空时共有$n+1$个位置，其排列数为$\mathrm{P}_x^{n+1}$。
    
\newpage

\subsubsection{复杂排列问题$03$}
    复杂排列问题$03$背景：存在$\lambda$个约束集合$X_1\sim X_\lambda$，其中的元素必须相邻。\\[3mm]
    复杂排列问题$03$公式：
    \begin{large}
        \begin{equation*}
...
\subsubsection{复杂排列问题$04$}
    复杂排列问题$04$背景：存在$\lambda$个约束集合$X_1\sim X_\lambda$，其中的元素必须隔开。\\[3mm]
    复杂排列问题$04$公式：
    \begin{large}
        \begin{equation*}
    ...
    依次类推即可，若任意一步出现空位不足，则该组数据无法用该方法求解，但不代表数据无解。\vspace{8pt}

\subsubsection{复杂排列问题$05$}
    复杂排列问题$05$背景：存在一个约束集合$X$，其中的元素分别不能置于某各不相同的位置上。\\[3mm]
    复杂排列问题$05$公式：
    \begin{large}
        \begin{equation*}
